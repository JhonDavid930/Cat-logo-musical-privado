...
- PUT /api/catalog: autenticación, comprobación Origin, tamaño máximo 10 MB, validación integral y transacción. Revisión obsoleta devuelve 409; entrada inválida, 400. No hay escrituras parciales. Tras guardar, elimina del almacenamiento privado los binarios que ya no están referenciados.
...
GET `/api/backup` exporta un ZIP portable con `catalog.json` y cada binario referenciado; POST restaura ZIP de hasta 8 GiB con autenticación, Origin y confirmación de sustitución en la interfaz. La importación valida estructura, rutas, recuentos, tamaños, tipos y SHA-256, prepara UUID nuevos y solo después sustituye metadatos en una transacción con control de revisión. Un fallo conserva el catálogo anterior y limpia los archivos preparatorios. Una escritura normal elimina los binarios retirados del catálogo después de confirmar la transacción; la restauración de una copia no ejecuta una limpieza global de archivos históricos. El ZIP contiene únicamente archivos referenciados por esa copia. La copia JSON sigue disponible, pero para trasladar binarios a otro equipo debe utilizarse el ZIP completo.

## CRUD y eliminación

El CRUD opera sobre las cuatro clases de entidad: composición, grabación, vídeo y lanzamiento. `removeEntity` elimina la entidad seleccionada y filtra relaciones entrantes/salientes, registros, créditos y documentos cuyo `entityId` le pertenece. No elimina otras entidades relacionadas: una grabación puede quitarse sin destruir su composición y un lanzamiento compartido permanece mientras no sea la ficha seleccionada. La interfaz exige escribir el título exacto antes de habilitar la acción. La API calcula los UUID de archivos que dejan de estar referenciados, guarda primero la nueva revisión y después retira binario y manifest del directorio privado.
