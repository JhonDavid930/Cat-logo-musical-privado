...
Actualizado: **2026-09-14**. Validación local real de Docker/PostgreSQL con datos sintéticos y repetición de las 28 pruebas unitarias/SQLite y TypeScript. Catálogo real intacto. Este documento permite retomar el proyecto sin leer la conversación.
...
| PostgreSQL y migraciones | Validados localmente el 14 de septiembre; migraciones 002/003 aplicadas dos veces sobre esquema sintético anterior. |
| Docker y volúmenes | Build/arranque, escritura como node, reinicio y restauración real en otra base/volumen superados. |
...
- Docker previsto: volúmenes `catalog_database` y `catalog_files`; TLS tiene volúmenes propios en la variante HTTPS.

Para abrir, sigue [README](../README.md): Node 24, `npm ci`, conservar .env.local y `npm run dev`. Abrir [loopback local](http://127.0.0.1:3000); esto no implica que el servidor siga activo desde la sesión anterior. Antes de modificar datos, obtener una copia ZIP y probar su recuperación en otro catálogo. JSON solo contiene metadatos: necesita los originales disponibles para recuperar referencias binarias.
...
NAS identificado: Synology DS225+, 2 GB RAM, Container Manager instalado. Solo inspeccionado. **El propietario exige validación local y aprobación antes de trasladar archivos o hacer Deploy al NAS.** Tampoco hay autorización implícita para Commit/Push, reinicios, Factory Reset o borrar volúmenes.
