...
El aviso de la captura del propietario se reprodujo: `POST /api/auth` con `Origin: http://localhost:3010` devolvía 403 aunque las llaves privadas estaban presentes y `Origin: http://127.0.0.1:3010` devolvía 401 con contraseña incorrecta. Tras la corrección, la contraseña local existente inicia sesión con 200 y cookie desde ambos alias. Una prueba Playwright con servidor aislado y contraseña sintética completó la navegación en Chrome desde `localhost`; `Origin: https://other.example` sigue rechazado con 403. Docker y PostgreSQL permanecen saludables con 210 entidades, 213 relaciones, 207 registros y revisión 11. Ninguna contraseña se registró en Git ni en las capturas de prueba.
